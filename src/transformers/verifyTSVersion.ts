...
export const verifyTSVersion = () => {
    if (ConfigLoader.load().unsafeTSVersion) {
        console.log(chalk.yellow(`WARN: You are enabled "${chalk.bold('unsafeTSVersion')}" flag, ${chalk.bold('dependency-injection-cat')} will not verify typescript version compatibility.`));
        return;
    }
