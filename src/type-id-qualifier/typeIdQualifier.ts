...
import { CLOSE_TYPE_ARGUMENTS_TOKEN, OPEN_TYPE_ARGUMENTS_TOKEN, TUPLE_TYPE_TOKEN } from './parseTokens';

export function typeIdQualifier(node: ts.Node): string {
    if (!isHasTypeNode(node) || node.type === undefined) {
        throw new Error(TypeQualifierError.HasNoType);
    }

    return getTypesNameDeep(node.type);
}
