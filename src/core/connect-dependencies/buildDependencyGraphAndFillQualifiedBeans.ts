        ...
        beanDescriptors.forEach(beanDescriptor => {
            ...
            dependencies.forEach(dependencyDescriptor => {
                const nonUniqBeanDescriptorsFromCurrentContext = dependencyDescriptor.qualifiedType.
                    typeIds.list().map(it => beansMap.get(it)).flat();

                const beanCandidatesFromCurrentContext = uniqNotEmpty(nonUniqBeanDescriptorsFromCurrentContext);

                beanCandidatesFromCurrentContext.length;
